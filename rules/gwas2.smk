...
source_dir = config.get('src_dir')
vcf_file = f'wgs/test/concat/{complete_region}.vcf.gz'
fws_file = f'wgs/test/concat/fws-{complete_region}/fws.tsv'
ibd_file = f'wgs/test/concat/ibd-{complete_region}/ibd.tsv'
dis_file = f'wgs/dist/concat/{complete_region}.vcf.gz'
...
rule prepare_samples:
    input:
        vcf_fn = vcf_file,
    output:
       samples = '{pfx}/{pheno}/vcf-samples.txt',
    shell:
...
rule vcf_to_bed:
    ...
    input:
        vcf_fn = vcf_file,
        sample_fn = '{pfx}/{pheno}/samples.txt',
    output:
        bed_fn = '{pfx}/{pheno}/variants.bed',
        bim_fn = '{pfx}/{pheno}/variants.bim',
        fam_fn = '{pfx}/{pheno}/variants.fam',
    params:
...
use rule vcf_to_bed as vcf_to_bed2 with:
    input:
        ...
        fws_fn = fws_file,
    output:
        bed_fn = '{pfx}/{pheno}/distance.bed',
        bim_fn = '{pfx}/{pheno}/distance.bim',
        fam_fn = '{pfx}/{pheno}/distance.fam',
...
rule prepare_phenotype_and_covars_file:
    ...
    input:
        ...
        sample_fn = '{pfx}/{pheno}/vcf-samples.txt',
    output:
...
rule run_fastlmm:
    threads: 12
    input:
        ped_fn = '{pfx}/{pheno}/variants.bed',
        dist_fn = '{pfx}/{pheno}/distance.bed',
        phenotype_fn = '{pfx}/{pheno}/phenotype.txt',
        covars_fn = '{pfx}/{pheno}/covars.txt'
    output:
        fn = '{pfx}/{pheno}/normal/gwas_results.feather',
    params:
...
rule plot_results:
    localrule: True
    input:
        fn = '{pfx}/{pheno}/{lmm_type}/gwas_results.feather',
    output:
        qq = '{pfx}/{pheno}/{lmm_type}/qq.png',
        mht = '{pfx}/{pheno}/{lmm_type}/mht.png',
    params:
