...
def init_argparser():
    ...
    p.add_argument('--covars-file', default=None,
                   help='covariates file in plink phenotype format')
    p.add_argument('--similarity-file', default=None,
                   help='SNPs file (in PED/binary PED) format for '
                   'similarity matrix')
...
def run_fastlmm(args):
    ...
    if args.similarity_file is None:
        results_df = single_snp_linreg(variant_data, args.phenotype_file,
                                       covar=args.covars_file, count_A1=False)
    else:
        results_df = single_snp(variant_data, args.phenotype_file, K0=K0,
                                covar=args.covars_file, count_A1=False)
